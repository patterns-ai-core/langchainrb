...
module Langchain::LLM
  ...
  class OpenAI < Base
    ...
    # @param stream [Boolean] Stream the response true/false; Block must be provided as well.
    # @return [String] The chat completion
    #
    def chat(prompt: "", messages: [], context: "", examples: [], stream: false, **options)
      ...
      if (streaming = stream && block_given?)
      ...
      raise "Chat completion failed: #{response}" if response.present? && response.dig("error")
