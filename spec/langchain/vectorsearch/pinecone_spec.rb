...
RSpec.describe Langchain::Vectorsearch::Pinecone do
  ...
  describe "add_texts" do
    ...
    before do
      ...
      allow(subject.llm).to receive_message_chain(:embed, :first_embedding).with(text: text).with(no_args).and_return(embedding)
  ...
  describe "#similarity_search_by_vector" do
    before do
      allow(subject.llm).to receive_message_chain(:embed, :first_embedding).with(text: text).and_return(embedding)
      allow(subject.client).to receive(:index).with(index_name).and_return(Pinecone::Index.new)
    end
